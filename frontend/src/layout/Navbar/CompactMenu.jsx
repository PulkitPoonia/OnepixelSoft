...
import socketService, { notificationQueue } from '../../utils/socketService';
...
const CompactMenu = ({ closeMenu, conversations }) => {
    ...
    // 🛡️ Deduplication ID Set
    const processedIds = useRef(new Set());
    const lastNotif = useRef({ title: '', body: '', time: 0 });
    ...
    useEffect(() => {
        window._pushNotification = (notif) => {
            ...
            // Dedupe by ID
            if (notif.id && processedIds.current.has(notif.id)) {
                console.log('🛡️ [DEDUPE] Ignoring identical ID:', notif.id);
                return;
            }
            
            // Dedupe by Content + Time
            if (
                notif.title === lastNotif.current.title &&
                notif.body === lastNotif.current.body &&
                (now - lastNotif.current.time) < 3000
            ) {
                console.log('🛡️ [DEDUPE] Ignoring identical content within 3s');
                return;
            }

            // Mark as processed
            if (notif.id) processedIds.current.add(notif.id);
            lastNotif.current = { title: notif.title, body: notif.body, time: now };
            ...
            // 🎵 Play notification sound (with check for primer)
            try {
                const player = audioPlayer.current;
                if (player) {
                    player.currentTime = 0;
                    player.volume = 0.6;
                    player.play().catch(e => console.log('🔊 Sound deferred (primer needed)'));
                }
            } catch (err) {
        ...
        // ✅ Process any queued notifications now that handler is ready
        setTimeout(() => {
            console.log('📨 Processing queued notifications, queue size:', notificationQueue.length);
            socketService.processNotificationQueue();
        }, 500);
                            ...
                            <div style={{ overflowY: 'auto', flex: 1 }}>
                                {notifications.length === 0 ? (
                                    ...
                                    <div 
                                        ...
                                        onClick={() => {
                                            if (n.link) {
                                                console.log('🚀 [NAV] Navigating to notification link:', n.link);
                                                navigate(n.link);
                                                setShowNotifications(false);
                                            }
                                        }}
