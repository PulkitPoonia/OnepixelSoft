  ...
  return async (dispatch) => {
    ...
    try {
      ...
      if (data.ok) {
        dispatch({
          type: LOAD_ALL_USERS,
          users: data.users || data.docs || [], // ✅ Robust handling for both formats
        });
      }
    } catch (err) {
...
export function updateConversation(conversationId, updateData) {
  return async (dispatch) => {
    const token = getToken();
    ...
    try {
      ...
      if (data.ok) {
        // Refresh conversations list to show updated name/image
        dispatch(loadConversations());
        return { ok: true };
      }
      return data;
