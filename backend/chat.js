...
/* ================= MEMORY ================= */
import { onlineUsers, userLastSeen, userSockets, offlineTimers } from './presenceStore.js';
import { deliverMissedNotifications } from './notificationStore.js';
...
const MAX_UPLOAD_SIZE = 500 * 1024 * 1024; // 500MB max upload limit
  ...
  try {
    ...
    if (req.method === 'GET' && pathname.match(/^\/api\/chat\/file\/[a-f0-9]{24}$/)) {
      try {
        ...
        // Look up file info first so we can serve the correct Content-Type
        ...
        // Serve with correct Content-Type so images/audio render inline
        res.writeHead(200, {
  ...
  try {
    ...
    if (req.method === 'GET' && pathname.match(/^\/api\/chat\/file\/[a-f0-9]{24}$/)) {
      try {
        ...
        // ✅ FIX: Allow any authenticated user to access chat files.
        // Files are shared in conversations — all participants need to view them.
        // The public download route already handles unauthenticated access.
    ...
    if (req.method === 'POST' && pathname === '/api/chat/upload') {
      try {
        ...
        const tempDir = path.join(path.dirname(fileURLToPath(import.meta.url)), 'uploads', 'temp');
        if (!fs.existsSync(tempDir)) fs.mkdirSync(tempDir, { recursive: true });
        const tempFilePath = path.join(tempDir, `chat_temp_${timestamp}_${randomStr}.tmp`);
        ...
        req.on('data', (chunk) => {
          ...
          if (!foundFile) {
            ...
            if (headerEnd !== -1) {
              ...
              if (fileNameMatch) {
                ...
                console.log('📁 Starting file streaming to temp file:', { fileName });
              }
            }
          }
          ...
          if (foundFile) {
            ...
            if (nextBoundaryIndex !== -1) {
              ...
              tempStream.write(dataToWrite);
              buffer = buffer.slice(nextBoundaryIndex + 2);
            } else if (buffer.length > 200) { // Keep a safe margin for boundary testing
              const safeSize = buffer.length - 100;
          ...
          tempStream.on('finish', async () => {
            try {
              ...
              if (fileSize > MAX_UPLOAD_SIZE) {
                ...
                res.writeHead(413);
                res.end(JSON.stringify({ ok: false, error: `File too large (${sizeMB}MB). Maximum size is ${maxMB}MB.` }));
                return;
              }
              ...
              // Detect content type from extension for correct inline rendering
              ...
              const detectedContentType = contentTypeMap[ext] || 'application/octet-stream';

              const LARGE_FILE_THRESHOLD = 50 * 1024 * 1024; // 50MB

              if (fileSize > LARGE_FILE_THRESHOLD) {
                // ✅ LARGE FILE: Move directly to disk at uploads/userfiles/
                const userFilesDir = path.join(path.dirname(fileURLToPath(import.meta.url)), 'uploads', 'userfiles');
                if (!fs.existsSync(userFilesDir)) fs.mkdirSync(userFilesDir, { recursive: true });

                const finalPath = path.join(userFilesDir, uniqueFileName);
                fs.rename(tempFilePath, finalPath, (moveErr) => {
                  if (moveErr) {
                    console.error('❌ Filesystem move error:', moveErr);
                    fs.unlinkSync(tempFilePath); // cleanup
                    if (!res.headersSent) {
                      res.writeHead(500);
                      res.end(JSON.stringify({ ok: false, error: 'Failed to save large file' }));
                    }
                    return;
                  }

                  const fileUrl = `/uploads/userfiles/${uniqueFileName}`;
                  console.log('✅ Large file saved to disk:', { fileName, filePath: finalPath, size: fileSize });
                  res.writeHead(200, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ ok: true, fileUrl, fileName, size: fileSize }));
                });

              } else {
                // ✅ SMALL FILE (<=50MB): Pipe into GridFS
                const bucket = new GridFSBucket(db);
...
export function initChatSocket(io, client) {
  ...
  io.on('connection', (socket) => {
    ...
    if (offlineTimers.has(username)) {
      console.log('⏳ [PRESENCE] Cleared offline timer for:', username);
      clearTimeout(offlineTimers.get(username));
      offlineTimers.delete(username);
    }

    // Mark online ONLY if first socket
    if (!onlineUsers.has(username)) {
      console.log('🟢 [PRESENCE] User came online:', username);
    ...
    deliverMissedNotifications(socket, db);


    socket.join(username);

    // Reuse the already declared db
    ...
    socket.on('send_message', async (data) => {
      ...
      // Emit to conversation room (real-time UI update)
      io.to(String(data.conversationId)).emit('receive_message', msgToSend);

      // 🔔 NEW: Persistent Push Notifications for participants
      if (conversation && conversation.participants) {
        (async () => {
          try {
            const senderUsername = username; // current socket user
            const isGroup = conversation.isGroup;
            const groupName = conversation.groupName;

            const notification = {
              icon: '💬',
              title: isGroup ? `${msg.senderName} in ${groupName}` : `Message from ${msg.senderName}`,
              body: msg.text || (msg.attachments?.length ? '📎 Attachment' : ''),
              isGroup: !!isGroup,
              link: isGroup 
                ? `/apps/chat/chat-groups?conversation=${data.conversationId}` 
                : `/apps/chat/chats?conversation=${data.conversationId}`
            };

            for (const p of conversation.participants) {
              const targetUsername = typeof p === 'string' ? p : p.username;
              
              // Don't notify the sender
              if (targetUsername === senderUsername) continue;

              if (onlineUsers.has(targetUsername)) {
                // If online, we don't necessarily NEED to emit 'push_notification' 
                // because 'receive_message' already triggers a notification on the client
                // if they are on a different page. 
                // However, for consistency with tasks, we can emit it too.
                console.log(`🔔 [NOTIF] User ${targetUsername} is online, skip queueing.`);
              } else {
                console.log(`📥 [NOTIF] Queuing offline chat notification for: ${targetUsername}`);
                await queueNotification(db, targetUsername, notification);
              }
            }
          } catch (err) {
            console.error('❌ [NOTIF] Error processing chat notifications:', err);
          }
        })();
      }
    ...
    socket.on('user_logging_out', () => {
      console.log('🚪 User logging out intentionally:', username);
      
      socket.isIntentionalLogout = true;
      
      // Immediately mark as offline
      if (userSockets.has(username)) {
        userSockets.get(username).clear();
      }
      ...
      console.log('🔴 [PRESENCE] User logged out and marked offline immediately');
    });

    /* ================= DISCONNECT ================= */
    socket.on('disconnect', () => {
      console.log('❌ Socket disconnected:', username, '| ID:', socket.id, '| intentional:', socket.isIntentionalLogout);

      const sockets = userSockets.get(username);
      if (sockets) {
        sockets.delete(socket.id);
        console.log(`📊 [PRESENCE] User ${username} has ${sockets.size} active sockets remaining`);
      }

      // If no sockets left, handle going offline
      if (!sockets || sockets.size === 0) {
        const lastSeen = new Date();
        userLastSeen.set(username, lastSeen);

        if (socket.isIntentionalLogout) {
          console.log('🔴 [PRESENCE] Intentional disconnect - marking offline immediately');
          onlineUsers.delete(username);
          io.emit('user_offline', { username, lastSeen });
        } else {
          // Unintentional disconnect: Start grace period to handle refreshes
          console.log('⏳ [PRESENCE] Unintentional disconnect - starting 10s grace period for:', username);
          
          const timerId = setTimeout(() => {
            // Re-check sockets count after grace period
            const currentSockets = userSockets.get(username);
            if (!currentSockets || currentSockets.size === 0) {
              console.log('🔴 [PRESENCE] Grace period expired. User marked offline:', username);
              onlineUsers.delete(username);
              io.emit('user_offline', { username, lastSeen: userLastSeen.get(username) || new Date() });
              offlineTimers.delete(username);
            } else {
              console.log('🟢 [PRESENCE] Grace period expired, but user reconnected. Staying online:', username);
            }
          }, 10000); // 10 seconds grace period

          offlineTimers.set(username, timerId);
        }
      }
    });
  });
