...
const Body = ({ showInfo }) => {
  ...
  // Set up socket listeners for comment events
  useEffect(() => {
    ...
    if (!socket) {
      console.error('❌ Socket service returned null');
      return;
    }

    console.log('🔌 Socket object obtained:', {
      socketId: socket.id,
      connected: socket.connected,
      url: socket.io?.engine?.hostname
    });

    // Attach connection event listener
    const handleConnect = () => {
      console.log('✅ [SOCKET CONNECT EVENT] Socket connected! Socket ID:', socket.id);
    };

    const handleDisconnect = () => {
      console.log('❌ [SOCKET DISCONNECT EVENT] Socket disconnected');
    };

    socket.on('connect', handleConnect);
    socket.on('disconnect', handleDisconnect);

    // Join all task rooms for each project and task
    const joinRooms = () => {
      let joinedCount = 0;
      projects.forEach(project => {
        project.tasks?.forEach(task => {
          const roomId = `task-${project.id}-${task.id}`;
          socket.emit('join', roomId);
          joinedCount++;
          console.log(`📦 [CLIENT] Emitting join for room: ${roomId}`);
        });
      });
      console.log(`✅ [CLIENT] Emitted join requests for ${joinedCount} task rooms`);
    };

    // Immediately try to join, or wait for connection
    if (socket.connected) {
      console.log('🔗 Socket already connected, joining rooms now...');
      joinRooms();
    } else {
    ...
    // Listen for comment added by other users
    const handleCommentAdded = (data) => {
      console.log('💬 [EVENT: comment_added] Received:', data);
      const { projectId, taskId, comment, author } = data;
      
      console.log(`   Checking: author.username="${author.username}" vs currentUsername="${currentUsername}"`);
      ...
      if (typeof window._pushNotification === 'function') {
        ...
        console.error('❌ window._pushNotification not available');
      }
    ...
    // Listen for comment updated by other users
    const handleCommentUpdated = (data) => {
      console.log('✏️ [EVENT: comment_updated] Received:', data);
      const { projectId, taskId, author } = data;
      
      console.log(`   Checking: author.username="${author.username}" vs currentUsername="${currentUsername}"`);
      
      // Don't notify if it's the current user
      if (author.username === currentUsername) {
        console.log('🤐 Skipping notification - comment edit is from current user');
        return;
      }

      console.log('📣 [NOTIFICATION] Showing comment_updated notification');
      // Show notification to other users
      if (typeof window._pushNotification === 'function') {
        window._pushNotification({
          icon: '✏️',
    ...
    // Listen for comment deleted by other users
    const handleCommentDeleted = (data) => {
      ...
      // Don't notify if it's the current user
      if (author.username === currentUsername) {
        console.log('🤐 Skipping notification - comment delete is from current user');
        return;
      }

      console.log('📣 [NOTIFICATION] Showing comment_deleted notification');
      // Show notification to other users
      if (typeof window._pushNotification === 'function') {
        window._pushNotification({
          icon: '🗑️',
    ...
    return () => {
      // Clean up: leave all task rooms
      console.log('🧹 [CLEANUP] Cleaning up socket listeners and rooms...');
      projects.forEach(project => {
        project.tasks?.forEach(task => {
          const roomId = `task-${project.id}-${task.id}`;
          socket.emit('leave', roomId);
          console.log(`🚪 [CLEANUP] Emitted leave for room: ${roomId}`);
        });
      });
  ...
  }, [projects, dispatch, currentUsername]);
                          ...
                          return newFiles.length > 0 ? (
                            <ListGroup className="file-list">
                              {newFiles.map(attachment => (
                                <ListGroup.Item key={attachment.id} className="d-flex justify-content-between align-items-center py-2">
                                  <div>
                                  ...
                                  <div className="d-flex gap-2">
                                    ...
                                    {(isAdmin || attachment.uploadedBy === currentUsername) && (
                      ...
                      {project.tasks && project.tasks.length > 0 ? (
                        <div className="tasks-list">
                          {project.tasks.map(task => (
                            <Card key={task.id} className="mb-2 task-card border-start border-4" style={{ borderLeftColor: `var(--bs-${getStatusColor(task.status)})` }}>
                              <Card.Header
                                className="cursor-pointer d-flex justify-content-between align-items-start bg-white"
                                    ...
                                    <div className="mb-3">
                                      ...
                                      <div className="subtasks-list">
                                        {task.subtasks.map(subtask => (
                                          <div key={subtask.id} className="d-flex align-items-center mb-2 p-2 bg-light rounded">
                                            <input
                                              type="checkbox"
          ...
          <Button
            ...
            onClick={async () => {
              if (commentText.trim()) {
                ...
                try {
                  await dispatch(addComment(commentProjectId, commentTaskId, commentText.trim()));
                  setShowCommentModal(false);
                  setCommentText('');
                } catch (e) {
