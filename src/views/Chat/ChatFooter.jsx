...
const ChatFooter = ({ conversationId, currentConversationId, replyingTo, clearReply }) => {
  ...
  const handleFileChange = async (e) => {
    ...
    if (file.size > maxSize) {
      setUploadError(`File is too large. Maximum size is 100MB. Your file: ${(file.size / 1024 / 1024).toFixed(2)}MB`);
      return;
    }
    ...
    try {
      ...
      if (data.ok && data.fileId) {
        ...
        console.log('[FILE STATE BEFORE SET]', { currentFileName: fileName });
        // Store the file ID from database
        setPreview(data.fileId); // Store fileId instead of URL
        console.log('[FILE STATE AFTER setPreview]', { previewSet: data.fileId });
        setFileName(data.fileName);
        console.log('[FILE STATE AFTER setFileName]', { fileNameSet: data.fileName });
      } else {
